...
local ThemePresets = {
    ...
    Mono       = { WHITE,             c3(70, 70, 70) },
    Monochrome = { WHITE,             c3(70, 70, 70) },
...
local AL = {
    hairline = 0.2329 * 1.6, card = 0.0197, cardStrk = 0.2329, tabFill = 0.04,
...
pcall(function() ContextActionService = game:GetService("ContextActionService") end)

local function toggleGameInputSink(block)
    if ContextActionService then
        if block then
            pcall(function()
                ContextActionService:BindActionAtPriority("INSui_BlockGameInput", function()
            ...
            pcall(function()
                ContextActionService:UnbindAction("INSui_BlockGameInput")
            end)
        end
    end
end
...
local function applyInputState(force)
    ...
    local m1Active = Input and Input.m1 and (Input.m1.click or Input.m1.held)
    if force or ProjectState.inputState ~= desired or (not desired and m1Active) then
        ProjectState.inputState = desired
        pcall(setrobloxinput, desired)
        toggleGameInputSink(not desired)
    end
end
...
local function snapValue(raw, item)
    local lo = item.min or 0
    local hi = item.max or 100
    local val = clamp(raw, lo, hi)
...
function ui:SetTheme(overrides)
    if type(overrides) == "string" then
        return ui:ApplyThemePreset(overrides)
...
local function processKeybinds()
    ...
    for _, item in ipairs(keybindItems) do
        ...
        elseif item.type == "keybind" and item.value and not item.listening and not isItemDisabled(item) then
            if item._pcSrc ~= item.value then item._pcSrc = item.value; item._pcMod, item._pcKey = parseCombo(item.value) end
            local mod, k = item._pcMod, item._pcKey
            local kIn = Input[k]
            local mIn = mod and Input[mod] or nil
            if kIn then
                local fire = (mod and (mIn.held and kIn.click)) or (not mod and kIn.click)
                if fire and item.callback then invoke(item.callback, item.value) end
            end
        end
    end
end
    ...
    pcall(function()
        ...
        elseif name == "Rain" then
            ...
            for i = 1, #p do local q = p[i]
                ...
                lineD(ax + (c0 - topy) / len * 2.5, c0, ax + (c1 - topy) / len * 2.5, c1, fxc or WHITE, 12, 1, aa)
            end
...
local function drawWindow(click, held, rightClick)
    ...
    local swC, swE = 56, max(145, floor(w * 0.235))
    ...
    rect(x, y, w, h, Theme.bg, 10, 8, (ProjectState.bgImg and 0.15 or baseAlpha) * v)
    if ProjectState.bgImg then
        pcall(function()
            ...
            if ProjectState.bgImgWFrac then
                ...
                bx = x + (w - bw) / 2; by = y + topH + ((h - topH) - bh) / 2
            else
                bw = w; bh = h
                bx = x; by = y
            end
            ProjectState.bgImg.Position = v2(bx, by); ProjectState.bgImg.Size = v2(bw, bh)
            ProjectState.bgImg.ZIndex = zord(20) + 1; ProjectState.bgImg.Transparency = (ProjectState.bgImgAlpha or 1.0) * v
    ...
    if ProjectState.resizable then
...
local function serializeConfig()
    ...
    for _, tab in ipairs(ProjectState.tabs) do
        for _, sec in ipairs(tab.sections) do
            for _, item in ipairs(sec.items) do
                if item.label and not item.noSave then
                    ...
                    elseif item.type == "keybind" then
                        data.flags[key] = item.value or false
                        data.keybinds[key] = { item.value or false, "Toggle" }
                    elseif item.type ~= "button" and item.type ~= "label" and item.type ~= "info" and item.type ~= "divider" and item.type ~= "image" then
                        data.flags[key] = item.value
                    end

                    if item.keybind then
                        data.keybinds[key] = { item.keybind.value or false, item.keybind.mode or "Toggle" }
                    end
...
local function applyConfigData(data)
        ...
        for _, sec in ipairs(tab.sections) do
            for _, item in ipairs(sec.items) do
                if item.label and not item.noSave then
                    ...
                    if fv ~= nil then
                        if item.type == "rangeslider" and type(fv) == "table" then
                        ...
                        elseif item.type == "colorpicker" and type(fv) == "table" then
                            item.value = c3((fv[1] or 1) * 255, (fv[2] or 1) * 255, (fv[3] or 1) * 255)
                            item.alpha = fv[4] or 1; invoke(item.callback, item.value, item.alpha)
                        elseif item.type == "dropdown" then
                            setDropdownValue(item, fv, true)
                        elseif item.type == "keybind" then
                            local kVal = (fv ~= false and fv ~= "none" and fv ~= "") and normalizeKey(fv) or nil
                            item.value = kVal
                        elseif item.type ~= "button" and item.type ~= "label" and item.type ~= "info" and item.type ~= "divider" and item.type ~= "image" then
                            setItemValue(item, fv, true)
                        end
                    end
                    local kb = data.keybinds and data.keybinds[key]
                    if kb then
                        local kVal = kb[1]
                        if kVal == nil and type(kb) == "table" then kVal = kb["1"] end
                        local kMode = kb[2]
                        if kMode == nil and type(kb) == "table" then kMode = kb["2"] end

                        local parsedKey = (kVal and kVal ~= false and kVal ~= "none" and kVal ~= "") and normalizeKey(kVal) or nil
                        if item.keybind then
                            item.keybind.value = parsedKey
                            if kMode then item.keybind.mode = normalizeMode(kMode) end
                        elseif item.type == "keybind" then
                            item.value = parsedKey
                        end
                    end
    ...
    if s then
        ...
        ProjectState.bgEffect = "Rain"
        ProjectState.bgEffectColor = WHITE
...
function ProjectState.imgBytes(target, legacy)
    local function isImageBytes(b)
        ...
        return (a == 0x89 and c == 0x50) or (a == 0xFF and c == 0xD8) or (a == 0x47 and c == 0x49) or (a == 0x52 and c == 0x49)
    end
    local b
    pcall(function()
        if readfile then
            local isF = false
            pcall(function() if isfile then isF = isfile(target) end end)
            if isF or not isfile then
                pcall(function() b = readfile(target) end)
            end
            if not isImageBytes(b) then
                pcall(function() b = readfile(target) end)
            end
        end
    end)
    ...
    b = nil; pcall(function() if isfile and isfile(cache) then b = readfile(cache) end end)
    if isImageBytes(b) then return b end
    if legacy then b = nil; pcall(function() if isfile and isfile(legacy) then b = readfile(legacy) end end)
        if isImageBytes(b) then pcall(function() if writefile then writefile(cache, b) end end); return b end
    end
    b = nil
    pcall(function()
        ...
        if req then
            ...
            if res and (res.StatusCode == 200 or res.Success or not res.StatusCode) and res.Body and #res.Body > 10 then
                if isImageBytes(res.Body) then b = res.Body end
            end
        end
        if not b and httpget then
            local res = httpget(target)
            if isImageBytes(res) then b = res end
        end
        if not b and game and game.HttpGet then
            local res = game:HttpGet(target, true)
            if isImageBytes(res) then b = res end
        end
    end)
    if isImageBytes(b) then pcall(function() if writefile then writefile(cache, b) end end); return b end
...
function ui:SetBackgroundImage(url, alpha, wFrac, hFrac)
    ProjectState.bgImgAlpha = alpha or 1.0
    ...
    if url ~= nil and url ~= "" then
        ...
        if #s > 24 and ((s1 == 0x89 and s2 == 0x50) or (s1 == 0xFF and s2 == 0xD8) or (s1 == 0x47 and s2 == 0x49) or (s1 == 0x52 and s2 == 0x49)) then
            ProjectState.bgImgUrl = nil
            pcall(function()
                local img = Drawing.new("Image")
                img.Data = s
                img.ZIndex = zord(20) + 1
                img.Transparency = ProjectState.bgImgAlpha or 1.0
                if ProjectState._winRect then
                    local r = ProjectState._winRect
                    local bw, bh, bx, by
                    if ProjectState.bgImgWFrac then
                        bw = ProjectState.bgImgWFrac * r.w; bh = (ProjectState.bgImgHFrac or 1) * (r.h - r.th)
                        bx = r.x + (r.w - bw) / 2; by = r.y + r.th + ((r.h - r.th) - bh) / 2
                    else
                        bw = r.w; bh = r.h
                        bx = r.x; by = r.y
                    end
                    img.Position = Vector2.new(bx, by)
                    img.Size = Vector2.new(bw, bh)
                    img.Visible = (r.v or 1) > 0.01
                else
                    img.Visible = false
                end
                ProjectState.bgImg = img
            end)
            return self
        end
    end
        ...
        pcall(function()
            ...
            if b and #b > 12 then
                if ProjectState.bgImgUrl == target then
                    local img = Drawing.new("Image"); img.Data = b
                    img.ZIndex = zord(20) + 1
                    img.Transparency = ProjectState.bgImgAlpha or 1.0
                    if ProjectState._winRect then
                        local r = ProjectState._winRect
...
local function applyThemeExtras(name)
    ...
    elseif name == "NeverBlox" then
        ...
        if not ProjectState.bgImgUrl and not ProjectState.bgImg then ui:SetBackgroundImage(nil) end
...
function ProjectState.buildSettingsTheme(tab)
...
function ProjectState.buildSettingsAppearance(tab)
...
function ProjectState.buildSettingsInterface(tab)
...
function ProjectState.buildSettingsConfigs(tab)
    local cf = tab:Section("Configs", "Left")
...
function ProjectState.buildSettingsSystem(tab)
    local sys = tab:Section("System", "Left")
...
function ProjectState.buildSettingsTab(win, icon)
    ...
    -- ProjectState.buildSettingsTheme(tab)
    -- ProjectState.buildSettingsAppearance(tab)
    ProjectState.buildSettingsInterface(tab)
    ProjectState.buildSettingsConfigs(tab)
    ProjectState.buildSettingsSystem(tab)

    return tab
end

function ProjectState.stripGlyphs(s)
    s = tostring(s or "")
...
function ui:CreateWindow(config)
    ...
    ProjectState.title = ProjectState.stripGlyphs(config.title or "uilib")
    if config.subtitle == "auto" then
        ...
        task.spawn(function()
            ...
            if type(nm) == "string" and nm ~= "" then ProjectState.subtitle = ProjectState.stripGlyphs(nm) end
        end)
    else
        ProjectState.subtitle = ProjectState.stripGlyphs(config.subtitle or "")
    end
    ...
    ProjectState.resizable = (config.resizable ~= false)
    ...
    if config.keybindOverlay ~= nil then ProjectState.hotkeyEnabled = config.hotkeyEnabled ~= false end
    ui:SetBackgroundEffect(config.backgroundEffect or "Rain")
    ProjectState.bgEffectColor = WHITE
    if config.theme then ui:SetTheme(config.theme) else ui:SetTheme("Mono") end
    if config.backgroundImage then ui:SetBackgroundImage(config.backgroundImage, config.backgroundOpacity or 1.0) end
    ...
    function win:AddSettingsTab(icon)
        ...
        ProjectState.settingsApi = ProjectState.buildSettingsTab(win, icon)
